...
public class Sort extends Iterator implements GlobalConst
{
   ...
   */
    private void setup_for_merge(int tuple_size, int n_R_runs) throws IOException, 
	   LowMemException, 
	   SortException,
	   Exception
    {
        ...
        for (int j=0; j<n_R_runs; j++) i_buf[j] = new SpoofIbuf(0, "*", "*", "*");
   ...
   */
    private int generate_runs(int max_elems, int sortFld, int sortFldLen) throws IOException, 
	   SortException,
	   MapUtilsException,
	   JoinsException,
	   Exception {
        ...
        switch (sortFld) {
            case 1:
                sortFldType = new AttrType(AttrType.attrString);
                break;
            case 2:
                sortFldType = new AttrType(AttrType.attrString);
                break;
            case 3:
                sortFldType = new AttrType(AttrType.attrInteger);
                break;
            case 4:
                sortFldType = new AttrType(AttrType.attrString);
                break;
        ...
        if(order.mapOrder == MapOrder.Ascending) {
            try {
                MIN_VAL(lastElem, sortFld);
            } catch (Exception e) {
                throw new SortException(e, "MIN_VAL failed");
            } 
        } else {
            try {
                MAX_VAL(lastElem, sortFld);
            } catch (Exception e) {
                throw new SortException(e, "MIN_VAL failed");
            } 
        }
   ...
   */
    private void MIN_VAL(Map lastElem, int sortFld) throws Exception {
   ...
   * @exception UnknowAttrType attrSymbol or attrNull encountered
   */
  private void MAX_VAL(Map lastElem, int sortFld) throws Exception {
   ...
   */
    public Map get_next() throws IOException, 
        ...
        if (first_time) {
            ...
            // generate runs
            Nruns = generate_runs(max_elems_in_heap, _sort_fld, sortFldLen);
