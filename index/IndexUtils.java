...
public class IndexUtils {
	...
	private static KeyClass getValue(CondExpr cd, AttrType type, int choice) throws UnknownKeyTypeException {
		...
		switch (type.attrType) {
			...
			case AttrType.attrInteger:
				if (choice == 1)
					return new IntegerKey(cd.operand1.integer);
				else
					return new IntegerKey(cd.operand2.integer);
			case AttrType.attrReal:
				/*
				 * // need FloatKey class in bt.java if (choice == 1) return new FloatKey(new
				 * Float(cd.operand.real)); else return new FloatKey(new
				 * Float(cd.operand.real));
				 */
			default:
